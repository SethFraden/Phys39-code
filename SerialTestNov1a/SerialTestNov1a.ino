...
public:
  ...
  // Constructor
  ArduinoParams(float T, float t, int pwm, int HC, float e, float I, float Ts, float T1, float T2, float T3) {
    temp = T;
    time = t;
    pwmNow = pwm;
    HeatCool = HC;
...
ArduinoParams ARDparams(20, 0, 0, 1, 0, 0, 20, 20, 20, 20);  // Initial Arduino parameters. Set initial time to zero
...
void loop() {
 ...
 float T1 = measureTemperature(tempIn1, 1000); // Measure thermistor at A0 pin
 float T2 = measureTemperature(tempIn2, 1000); // Measure thermistor at A0 pin
 float T3 = measureTemperature(tempIn3, 1000); // Measure thermistor at A0 pin
